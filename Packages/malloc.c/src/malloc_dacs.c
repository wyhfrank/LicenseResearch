...
static const char revid[] =
  "$Id: malloc.c 2472 2010-06-11 17:35:04Z brachman $";
#endif
...
#ifdef HISTO
#include "/k/brachman/src/lib/histo/histo.h"
...
#ifdef HISTO
static long vec[] = {
  8, 16, 32, 64, 128, 256, 512, 1024, 2048, 4096, 8192, 16384, 32768, 65536, 0
};
static unsigned int counters[20];
...
if (!did_init) {
int i;
for (i = 0; i < 20; i++) counters[i] = 0;
  histo_inew(&h, 0, 0, 0, vec, 0, HISTO_UINT_COUNTER, counters);
  dacs_atexit(malloc_end, NULL);
did_init = 1;
}
histo_i(&h, size, 1);
#endif
